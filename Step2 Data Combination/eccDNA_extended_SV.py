...
def merge_npz_files(input_files, output_file):
    ...
    # 保存合并后的结果和掩码数组
    np.savez_compressed(output_file, DNASV=result, DNASVmask=mask)

def main():
    # 设置命令行参数解析
    parser = argparse.ArgumentParser(description='Merge multiple extended SV NPZ files into a single NPZ file.')
    ...
    print(f"Successfully merged {len(args.input)} extended SV files into {args.output}")
