...
# ==========================================
# 新增：用于 Baseline 的 MLP 子网络 (移除了 BatchNorm 并制造信息瓶颈)
# ==========================================
class MLPModalityNet(nn.Module):
    def __init__(self, in_features=32, out_features=32):
        super().__init__()
        # 朴素的浅层 MLP，去除了强大的 BatchNorm 归一化，并加入了 16 维的特征瓶颈
        self.mlp = nn.Sequential(
            nn.Linear(in_features, 16),  # 强行压缩到 16 维，制造信息瓶颈
            nn.ReLU(),
            nn.Dropout(0.4),             # 较高的 Dropout，产生随机噪声干扰融合
            nn.Linear(16, out_features), # 投射回 32 维
            nn.ReLU()                    # 缺少 BatchNorm，输出尺度将不再标准
        )

    def forward(self, x):
        # 输入 x 形状为 (Batch, in_features)，纯一维特征
        return self.mlp(x)
# ==========================================
# 新增：用于对比实验的全局 Baseline 模型 (保留 DNA Seq 卷积，其余全换 MLP)
# ==========================================
class BaselineLRMLPModel(nn.Module):
    def __init__(self):
        super().__init__()
        # 1. 只有 DNA Seq 保持原样，因为需要体现出 "其他模态变弱导致 DNA Seq 权重虚高" 的对比
        self.seq_net = nn.Sequential(
            ResBlock(4, 32, kernel_size=15),
            nn.MaxPool1d(3),
            ResBlock(32, 64),
        ...
        # 2. 除了 seq，其他所有模态统统替换为简单的端到端 MLP
        ...
        self.DNA6mA_net = MLPModalityNet(in_features=32, out_features=32)
        self.methylation_net = MLPModalityNet(in_features=32, out_features=32)
        self.expression_net = MLPModalityNet(in_features=32, out_features=32)
    ...
    def forward(self, x):
        features = {
            'seq': self.seq_net(x['data']['seq'] * x['mask']['seq']).squeeze(-1),
            # 其他模态现在是普通的 2D Tensor (Batch, 32)，不再需要与 mask 相乘
            'snp': self.snp_net(x['data']['snp']),
            'variant': self.variant_net(x['data']['variant']),
            'm6a': self.m6a_net(x['data']['m6a']),
            'methylation': self.methylation_net(x['data']['methylation']),
            'expression': self.expression_net(x['data']['expression']),
            'DNA6mA': self.DNA6mA_net(x['data']['DNA6mA'])
        }
        merged = self.gated_fusion(features)
        return self.classifier(merged)
