...
def read_af_file(af_path):  
    """Reads a file containing values corresponding to base positions and returns a dictionary where keys are (chromosome, position) and values are the corresponding AF values. Positions with NA or no records are set to 0."""  
    af_dict = {}  
...
def bed_to_input(bed_path, af_dict, output_path):  
    """Reads a BED file, generates an encoding array for each line, and saves them into an NPZ file."""  
    # Read BED file  
    ...
    for index, row in bed_df.iterrows():  
        ...
        # Iterate through each position to generate the encoding array  
        current_row_enc = []  
        ...
        # Convert the encoding of the row to a numpy array  
